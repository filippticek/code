...
def adrese():
    string = "3 "
    string2 = "3"
    portovi = ["vat", "6789", "prosharerequest", "2222", "3333", "5555"]
...
PORTS = "1111 2222 dec-notes".split(' ')
...
def naredbe(udp_sock):
    ...
    if naredba == "r\n":
	    ...
	    leng = udp_sock.sendto(POPIS3_send, (adresa, port))
    elif naredba == "pt\n":
        print "--> PROG_TCP"
        for (adresa, port) in botovi:
	    ...
	    leng = udp_sock.sendto('4', (adresa, port))
    elif naredba == "q\n":
        print "--> QUIT"
        for (adresa, port) in botovi:
            print "%s:%d <--- QUIT" % (adresa, port)
	    leng = udp_sock.sendto('0', (adresa, port))
        print "Kraj programa."
        sys.exit(0)
    elif naredba == "h\n":
        help()
