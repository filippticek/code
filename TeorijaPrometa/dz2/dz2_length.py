...
def best_fit_distribution(data, bins=50, ax=None):
    ...
    DISTRIBUTIONS = [st.alpha,st.anglit,st.arcsine,st.beta,st.betaprime,st.bradford,st.burr,st.cauchy,st.chi,st.chi2,st.cosine,
        ...
        st.logistic,st.loggamma,st.loglaplace,st.lognorm,st.lomax,st.maxwell,st.nakagami,st.ncx2,
        st.nct,st.norm,st.pareto,st.pearson3,st.powerlognorm,st.powernorm,st.rdist,st.reciprocal,
        st.rayleigh,st.rice,st.recipinvgauss,st.semicircular,st.t,st.triang,st.truncexpon,st.truncnorm,st.tukeylambda,
        st.uniform,st.vonmises,st.vonmises_line,st.wald,st.weibull_min,st.weibull_max,st.wrapcauchy]
    '''  

    DISTRIBUTIONS = [st.expon,st.norm, st.dweibull,st.mielke,st.powerlaw,st.gumbel_l]
...
data_array=[]
with open('/home/filip/code/TeorijaPrometa/dz2/youtube_length.csv') as csvDataFile:
...
ax2 = data.plot(kind='hist', bins=500,  alpha=0.5)
